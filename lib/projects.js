...
export async function getAllProjects() {
    ...
    const projects = await Project.find({ status: { $ne: 'rejected' }, assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
    return projects;
}
...
export async function getAllRunningProjects() {
    try {
        ...
        const user = await getUser();
        if (user?.name === 'Muneeb Ur Rehman' || user?.name === 'Nabeel Ahmad' || user?.name === 'John Doe') {
            const projects = await Project.find({ status: "in-progress", assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
            return projects;
        } else {
            const projects = await Project.find({ status: "in-progress", assignees: user?._id })
                .populate({
                    path: "createdBy",
            ...
            // keep only projects whose creator is not private
            return projects.filter(p => p.createdBy);
...
export async function getAllCompletedProjects() {
    try {
        ...
        const user = await getUser();
        if (user?.name === 'Muneeb Ur Rehman' || user?.name === 'Nabeel Ahmad' || user?.name === 'John Doe') {
            const projects = await Project.find({ status: "completed", assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
            return projects;
        } else {
            const projects = await Project.find({ status: "completed", assignees: user?._id })
                .populate({
                    path: "createdBy",
            ...
            // remove projects whose creator is private
            return projects.filter(p => p.createdBy);
        }
    } catch (error) {
        console.error(error.message);
...
export async function getAllArchivedProjects() {
    try {
        ...
        if (user?.name === 'Muneeb Ur Rehman' || user?.name === 'Nabeel Ahmad' || user?.name === 'John Doe') {
            const projects = await Project.find({ status: "archived", assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
            return projects;
        } else {
            const projects = await Project.find({ status: "archived", assignees: user?._id })
                .populate({
                    path: "createdBy",
                    match: { isPrivate: false },
                })
            ...
            // drop projects whose creator is private
            return projects.filter(p => p.createdBy);
...
export async function getEveryProject() {
    try {
        ...
        const user = await getUser();
        if (user?.name === 'Muneeb Ur Rehman' || user?.name === 'Nabeel Ahmad' || user?.name === 'John Doe') {
            const projects = await Project.find({ assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
            return projects;
        } else {
            const projects = await Project.find({ assignees: user?._id })
                .populate({
                    path: "createdBy",
                    match: { isPrivate: false },
                })
...
export async function getAllPendingProjects() {
    try {
        ...
        const user = await getUser();
        if (user?.name === 'Muneeb Ur Rehman' || user?.name === 'Nabeel Ahmad' || user?.name === 'John Doe') {
            const projects = await Project.find({ status: "pending", assignees: user?._id }).populate('createdBy').sort({ createdAt: -1 }).lean();
            return projects;
        } else {

            const projects = await Project.find({ status: "pending", assignees: user?._id })
                .populate({
                    path: "createdBy",
                    match: { isPrivate: false },
                })
            ...
            // keep only projects whose creator is public
            return projects.filter(p => p.createdBy);
...
export async function getAssignableUsers() {
    try {
        ...
        const users = await User.find(
            ...
            "_id name email role companyName"
        ).lean();
