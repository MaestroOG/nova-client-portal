    ...
    <>
      ...
      <Container className="bg-white p-4 rounded-lg">
        ...
        <div className="grid grid-cols-1 items-stretch md:grid-cols-4 mt-5 gap-4">
          ...
          {projects?.length === 0 && (
            <div className="p-6">
              {user?.role !== 'manager' && <span className="dark:text-background">No Projects For Now!</span>}
              {user?.role === 'manager' && <span className="dark:text-background">Wait for your first project to be assigned!</span>}
            </div>
          )}
